    ...
    <div class="flex flex-col items-center mt-8">
        <div class="w-32 h-32 rounded-full overflow-hidden">
            @if ($user->profile?->profile_pic)
                <img class="object-cover w-full h-full" src="{{ Storage::url($user->profile->profile_pic) }}"
                    alt="User Profile Picture">
            @else
                <div class="bg-gray-300 flex items-center justify-center w-full h-full text-gray-600 text-xl">No Picture
                </div>
            @endif
        </div>
        <div class="mt-4">
                    ...
                    <li class="flex items-center justify-between bg-white rounded-lg shadow-md p-4 mb-4">
                        ...
                        @if (Auth::check() &&
                            ...
                            <form class="inline-block" method="POST"
                                ...
                                @method('DELETE')
                                <button type="submit"
                                    class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-md transition duration-200">Delete</button>
                            </form>
                        @endif
                    </li>
                @endforeach
